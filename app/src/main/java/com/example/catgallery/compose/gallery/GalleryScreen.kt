...
import com.example.catgallery.compose.common.ErrorListState
...
private fun SearchBar() {
    Row(
        ...
        Button(
            onClick = {}, modifier = Modifier.size(45.dp), contentPadding = PaddingValues(
                all = 0.dp
            )
        ) {
            Icon(
...
@Composable
private fun Gallery(
    cats: Flow<PagingData<GalleryItemVM>>,
    onFavouriteClick: (GalleryItemVM) -> Unit,
    ...
    Column {
        LazyVerticalGrid(GridCells.Fixed(2), modifier = Modifier.fillMaxSize()) {
            items(count = pagingItems.itemCount, key = { index ->
                ...
                GalleryItem(cat,
                    onFavouriteClick = { onFavouriteClick(cat) },
                    onDownloadClick = { onDownloadClick(cat.value.url) })
            }
        }
        when (pagingItems.loadState.append) { // Pagination
            is LoadState.Error -> {
                Toast.makeText(
                    ...
                    stringResource(id = R.string.error_pagination),
                    Toast.LENGTH_SHORT
                ).show()
            }
